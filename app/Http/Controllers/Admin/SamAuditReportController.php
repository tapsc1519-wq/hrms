    ...
    public function index()
    {
        ...
        $stats = [
            ...
            'policy_exceptions_expiring' => SoftwarePolicyException::where('organization_id', $organizationId)
                ...
                ->whereBetween('expires_at', [now()->toDateString(), now()->addDays(14)->toDateString()])
                ->count(),
    ...
    private function writeManifest(string $directory, Carbon $generatedAt): void
    {
        $this->csv($directory, '99-manifest.csv', ['File Name','Size Bytes','Data Rows','SHA-256','Generated At'], function ($handle) use ($directory, $generatedAt) {
            collect(File::files($directory))
                ...
                ->each(function ($file) use ($handle, $generatedAt) {
                    fputcsv($handle, [
                        $file->getFilename(),
                        $file->getSize(),
