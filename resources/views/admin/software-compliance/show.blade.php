...
<div class="table-card mb-3">
    ...
    @forelse($exceptions as $exception)@php $isExpiredException = $exception->status === 'approved' && $exception->days_to_expiry < 0; $isExpiringException = $exception->status === 'approved' && $exception->days_to_expiry >= 0 && $exception->days_to_expiry <= 14; @endphp<tr class="{{ $isExpiredException ? 'table-danger' : ($isExpiringException ? 'table-warning' : '') }}"><td class="ps-4"><div class="fw-bold">{{ $exception->user?->name ?? 'No user mapped' }}</div><div class="text-muted small">{{ $exception->asset?->asset_tag ?? $exception->discovery?->raw_name ?? 'No device mapped' }}</div></td><td>{{ $exception->valid_from->format('d M Y') }} to {{ $exception->expires_at->format('d M Y') }}<div class="{{ $isExpiredException ? 'text-danger fw-semibold' : 'text-muted' }} small">{{ $isExpiredException ? abs($exception->days_to_expiry).' day(s) overdue' : $exception->days_to_expiry.' day(s) to expiry' }}</div></td><td><div>{{ $exception->reason }}</div>@if($exception->conditions)<div class="text-muted small">Conditions: {{ $exception->conditions }}</div>@endif</td><td>{{ $exception->approvedBy?->name ?? 'Unknown' }}<div class="text-muted small">{{ $exception->created_at->format('d M Y') }}</div></td><td><span class="badge bg-{{ $exception->status_badge }}">{{ $exception->status_label }}</span><div class="mt-1"><span class="badge bg-{{ $exception->expiry_badge }}">{{ $exception->expiry_label }}</span></div></td><td class="text-end pe-4">@if($exception->status === 'approved')<div class="d-inline-flex gap-1 flex-wrap justify-content-end"><button type="button" class="btn btn-sm {{ $isExpiredException ? 'btn-danger' : 'btn-outline-primary' }} extend-exception-button" data-bs-toggle="modal" data-bs-target="#extendPolicyExceptionModal" data-action="{{ route('admin.software-compliance.policy-exceptions.extend', [$software, $exception]) }}" data-title="{{ $exception->user?->name ?? $exception->asset?->asset_tag ?? $exception->discovery?->raw_name ?? 'Policy exception' }}" data-expires="{{ $exception->expires_at->toDateString() }}" data-reason="{{ $exception->reason }}" data-conditions="{{ $exception->conditions }}">{{ $isExpiredException ? 'Renew' : 'Extend' }}</button>@if($exception->is_active)<form method="POST" action="{{ route('admin.software-compliance.policy-exceptions.revoke', [$software, $exception]) }}" onsubmit="return confirm('Revoke this policy exception?')">@csrf @method('PATCH')<button class="btn btn-sm btn-outline-danger">Revoke</button></form>@endif</div>@else<span class="text-muted small">Closed</span>@endif</td></tr>
...
<div class="modal fade" id="extendPolicyExceptionModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog"><form id="extendPolicyExceptionForm" method="POST" class="modal-content">@csrf @method('PATCH')<div class="modal-header"><div><h5 class="modal-title mb-0">Extend Policy Exception</h5><small id="extendPolicyExceptionTarget" class="text-muted"></small></div><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="alert alert-info small"><i class="bi bi-shield-check me-1"></i>Extending an exception keeps the detected installation approved and cancels open uninstall remediation for that discovery record.</div><div class="mb-3"><label class="form-label">New Expiry Date</label><input id="extendExceptionExpires" type="date" name="expires_at" class="form-control" min="{{ today()->toDateString() }}" value="{{ today()->addDays(30)->toDateString() }}" required><div id="extendExceptionCurrent" class="form-text"></div></div><div class="mb-3"><label class="form-label">Business Reason</label><textarea id="extendExceptionReason" name="reason" rows="3" maxlength="2000" class="form-control" required></textarea></div><div><label class="form-label">Conditions</label><textarea id="extendExceptionConditions" name="conditions" rows="2" maxlength="2000" class="form-control"></textarea></div></div><div class="modal-footer"><button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button><button class="btn btn-primary"><i class="bi bi-calendar-plus me-1"></i>Extend Exception</button></div></form></div></div>
