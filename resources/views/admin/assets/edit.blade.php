...
<div class="page-header">
    <a href="{{ route('admin.assets.show', $asset) }}" class="back-link"><i class="bi bi-arrow-left"></i> {{ $asset->name }}</a>
    <h4>Edit Asset</h4>
    <p>Update the details for <strong>{{ $asset->asset_tag ?? $asset->name }}</strong>.</p>
</div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="asset_brand_id" id="assetBrandSelect" class="form-select" onchange="filterModels()">
                            ...
                            <option value="{{ $b->id }}" data-name="{{ $b->name }}"
                                    {{ old('asset_brand_id', $asset->asset_brand_id) == $b->id ? 'selected':'' }}>{{ $b->name }}</option>
                            @endforeach
                            <option value="__custom__" {{ !$asset->asset_brand_id && $asset->brand ? 'selected':'' }}>Other (type below)</option>
                        </select>
                        <input type="text" name="brand" id="brandCustom" class="form-control mt-1"
                               value="{{ old('brand', $asset->brand) }}" placeholder="Type brand name…"
                               style="display:{{ (!$asset->asset_brand_id && $asset->brand) ? 'block':'none' }}">
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="asset_model_id" id="assetModelSelect" class="form-select" onchange="onModelSelect(this)">
                            ...
                            <option value="{{ $m->id }}"
                                    ...
                                    {{ old('asset_model_id', $asset->asset_model_id) == $m->id ? 'selected':'' }}>
                                {{ $m->name }}
                            </option>
                            @endforeach
                            <option value="__custom__" {{ !$asset->asset_model_id && $asset->model ? 'selected':'' }}>Other (type below)</option>
                        </select>
                        <input type="text" name="model" id="modelCustom" class="form-control mt-1"
                               value="{{ old('model', $asset->model) }}" placeholder="Type model name…"
                               style="display:{{ (!$asset->asset_model_id && $asset->model) ? 'block':'none' }}">
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="serial_number" class="form-control"
                               value="{{ old('serial_number', $asset->serial_number) }}" placeholder="From device label">
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="col-12" id="assetSpecSection" style="{{ $asset->category_id ? '' : 'display:none' }}">
                        ...
                        <div id="assetSpecFields" class="row g-2">
                            {{-- Populated by JS with existing values --}}
                        </div>
                    </div>
                    <div class="col-12">
                        ...
                        <textarea name="specifications" class="form-control form-control-sm" rows="2"
                                  placeholder="Any extra details…">{{ old('specifications', $asset->specifications) }}</textarea>
                    </div>
                    <div class="col-12">
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <select name="vendor_id" class="form-select">
                            ...
                            <option value="{{ $v->id }}" {{ old('vendor_id', $asset->vendor_id) == $v->id ? 'selected':'' }}>
                                {{ $v->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="purchase_price" class="form-control"
                               value="{{ old('purchase_price', $asset->purchase_price) }}" step="0.01" min="0" placeholder="0.00">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Warranty Expiry</label>
                        <input type="date" name="warranty_expiry_date" class="form-control"
                               value="{{ old('warranty_expiry_date', $asset->warranty_expiry_date?->format('Y-m-d')) }}">
                    </div>
                    <div class="col-md-8">
                        <label class="form-label">Warranty Terms</label>
                        <input type="text" name="warranty_terms" class="form-control"
                               value="{{ old('warranty_terms', $asset->warranty_terms) }}" placeholder="e.g. 3-year on-site warranty">
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <select name="dep_method" class="form-select">
                            ...
                            <option value="straight_line"     {{ old('dep_method', $asset->dep_method) == 'straight_line'     ? 'selected':'' }}>Straight Line</option>
                            <option value="declining_balance" {{ old('dep_method', $asset->dep_method) == 'declining_balance' ? 'selected':'' }}>Declining Balance</option>
                            <option value="sum_of_years"      {{ old('dep_method', $asset->dep_method) == 'sum_of_years'      ? 'selected':'' }}>Sum of Years</option>
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Useful Life (Years)</label>
                        <input type="number" name="useful_life_years" class="form-control"
                               value="{{ old('useful_life_years', $asset->useful_life_years ?? 3) }}" min="1" max="50">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Salvage Value (&#8377;)</label>
                        <input type="number" name="salvage_value" class="form-control"
                               value="{{ old('salvage_value', $asset->salvage_value ?? 0) }}" step="0.01" min="0">
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <select name="condition" class="form-select" required>
                            ...
                            <option value="{{ $c }}" {{ old('condition', $asset->condition) == $c ? 'selected':'' }}>
                                {{ ucfirst($c) }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="notes" class="form-control" rows="3"
                                  placeholder="Any additional notes…">{{ old('notes', $asset->notes) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="form-card-body text-center">
                ...
                <div class="mb-3">
                    <img src="{{ asset('storage/' . $asset->image) }}"
                         class="rounded-3 border" style="max-height:120px;max-width:100%;object-fit:contain">
                ...
                <div class="border rounded-3 p-3 bg-light" style="border-style:dashed!important;border-color:#e2e8f0!important">
                    ...
                    <small class="text-muted d-block mt-1">{{ $asset->image ? 'Upload to replace current image' : 'JPG, PNG, max 2MB' }}</small>
                </div>
            </div>
        </div>
...
<div class="form-actions" style="border-radius:14px;border:1px solid #e2e8f0;margin-top:.5rem">
    <a href="{{ route('admin.assets.show', $asset) }}" class="btn-cancel">Cancel</a>
...
// Existing asset's saved specs (passed from PHP)
const existingSpecs = @json($asset->specs ?? []);
