...
<div class="alert alert-info d-flex gap-2 align-items-start">
    ...
    <div><strong>Approved software demand has been added.</strong> The selected employee requests will be linked to this PO and fulfilled as license seats are received.</div>
</div>
@endif
                ...
                <div id="itemsContainer">
                    ...
                    <div class="item-row rounded-3 mb-3 p-3" id="item-{{ $index }}" style="background:#f8fafc;border:1.5px solid #e2e8f0" data-index="{{ $index }}">
                        ...
                        <div class="row g-2 align-items-end">
                            ...
                            <div class="col-md-3 asset-field {{ $type === 'software' ? 'd-none' : '' }}"><label class="form-label">Asset Category</label><select name="items[{{ $index }}][category_id]" class="form-select"><option value="">None</option>@foreach($categories as $category)<option value="{{ $category->id }}" @selected(($item['category_id'] ?? null) == $category->id)>{{ $category->name }}</option>@endforeach</select></div>
