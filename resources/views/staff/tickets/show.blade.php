@extends('layouts.app')
@section('title', $ticket->ticket_number)

@section('content')
<div class="page-header d-flex justify-content-between align-items-start">
    <div>
        <a href="{{ route('staff.tickets.index') }}" class="back-link"><i class="bi bi-arrow-left"></i> My Tickets</a>
        <h4>{{ $ticket->ticket_number }}</h4>
        <p>{{ $ticket->subject }}</p>
    </div>
    <span class="badge fs-6 mt-1 bg-{{ $ticket->status_badge }}">{{ $ticket->status_label }}</span>
</div>

@if(session('success'))
<div class="alert alert-success alert-dismissible fade show rounded-3 mb-3">
    <i class="bi bi-check-circle me-2"></i>{{ session('success') }}
    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
</div>
@endif

<div class="row g-4">

    {{-- LEFT: Thread --}}
    <div class="col-lg-8">

        {{-- Original ticket --}}
        <div class="form-card mb-3">
            <div class="form-card-header" style="justify-content:space-between">
                <div style="display:flex;align-items:center;gap:.6rem">
                    <span class="icon-wrap icon-blue"><i class="bi {{ $ticket->category_icon }}"></i></span>
                    <div>
                        <div style="font-weight:700;color:#0f172a;font-size:.9rem">{{ $ticket->subject }}</div>
                        <div style="font-size:.75rem;color:#94a3b8">Submitted {{ $ticket->created_at->format('d-m-Y H:i') }}</div>
                    </div>
                </div>
                <span class="badge bg-{{ $ticket->priority_badge }}">{{ ucfirst($ticket->priority) }}</span>
            </div>
            <div class="form-card-body">
                <div style="white-space:pre-line;color:#334155;font-size:.875rem;line-height:1.7">{{ $ticket->description }}</div>
                @if($ticket->asset)
                <div class="mt-3 pt-3 border-top d-flex align-items-center gap-2" style="font-size:.82rem;color:#64748b">
                    <i class="bi bi-box-seam text-primary"></i>
                    Related asset: <strong>{{ $ticket->asset->name }}</strong>
                    @if($ticket->asset->asset_tag) ({{ $ticket->asset->asset_tag }}) @endif
                </div>
                @endif
                @include('partials._ticket_attachments', ['attachments' => $ticket->attachments])
            </div>
        </div>

        {{-- Replies thread --}}
        @foreach($replies as $reply)
        @php $isMe = $reply->user_id === auth()->id(); @endphp
        <div class="mb-3 d-flex gap-3 {{ $isMe ? 'flex-row-reverse' : '' }}">
            {{-- Avatar --}}
            <div class="flex-shrink-0" style="width:36px;height:36px;border-radius:50%;background:{{ $isMe ? '#dbeafe' : '#f1f5f9' }};color:{{ $isMe ? '#1d4ed8' : '#475569' }};display:flex;align-items:center;justify-content:center;font-weight:700;font-size:.85rem">
                {{ strtoupper(substr($reply->user->name, 0, 1)) }}
            </div>
            {{-- Bubble --}}
            <div style="max-width:80%">
                <div class="rounded-3 px-3 py-2"
                     style="background:{{ $isMe ? '#eff6ff' : '#f8fafc' }};border:1.5px solid {{ $isMe ? '#bfdbfe' : '#e2e8f0' }}">
                    <div style="font-size:.72rem;color:#94a3b8;margin-bottom:.25rem">
                        <strong style="color:#334155">{{ $isMe ? 'You' : $reply->user->name }}</strong>
                        · {{ $reply->created_at->format('d-m-Y H:i') }}
                    </div>
                    <div style="font-size:.875rem;color:#334155;white-space:pre-line;line-height:1.6">{{ $reply->message }}</div>
                    @include('partials._ticket_attachments', ['attachments' => $reply->attachments])
                </div>
            </div>
        </div>
        @endforeach

        {{-- Reply box --}}
        @if($ticket->is_open)
        <div class="form-card">
            <div class="form-card-header">
                <span class="icon-wrap icon-green"><i class="bi bi-reply"></i></span>
                Add a Reply
            </div>
            <div class="form-card-body">
                <form action="{{ route('staff.tickets.reply', $ticket) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <textarea name="message" class="form-control @error('message') is-invalid @enderror"
                              rows="4" required
                              placeholder="Type your reply or provide additional information…"></textarea>
                    @error('message')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    <div class="mt-3">
                        @include('partials._attachment_upload', ['inputName' => 'attachments[]', 'zoneId' => 'staff-reply'])
                        @error('attachments.*')<div class="text-danger mt-1" style="font-size:.82rem">{{ $message }}</div>@enderror
                    </div>
                    <div class="mt-3 d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-send me-1"></i>Send Reply
                        </button>
                    </div>
                </form>
            </div>
        </div>
        @else
        <div class="rounded-3 px-4 py-3 text-center text-muted" style="background:#f8fafc;border:1.5px dashed #e2e8f0;font-size:.85rem">
            <i class="bi bi-lock d-block mb-1 fs-5"></i>
            This ticket is <strong>{{ $ticket->status_label }}</strong>. No further replies can be added.
        </div>
        @endif

    </div>

    {{-- RIGHT: Info panel --}}
    <div class="col-lg-4">

        <div class="form-card">
            <div class="form-card-header">
                <span class="icon-wrap icon-amber"><i class="bi bi-info-circle"></i></span>
                Ticket Information
            </div>
            <div class="form-card-body">
                <dl style="font-size:.82rem;margin:0;display:grid;row-gap:.8rem">
                    <div>
                        <dt style="color:#94a3b8;font-size:.7rem;text-transform:uppercase;letter-spacing:.5px;font-weight:700">Ticket #</dt>
                        <dd style="font-family:monospace;font-weight:700;color:#3b82f6;margin:0">{{ $ticket->ticket_number }}</dd>
                    </div>
                    <div>
                        <dt style="color:#94a3b8;font-size:.7rem;text-transform:uppercase;letter-spacing:.5px;font-weight:700">Category</dt>
                        <dd style="margin:0;color:#334155;font-weight:600"><i class="bi {{ $ticket->category_icon }} me-1"></i>{{ $ticket->category_label }}</dd>
                    </div>
                    <div>
                        <dt style="color:#94a3b8;font-size:.7rem;text-transform:uppercase;letter-spacing:.5px;font-weight:700">Priority</dt>
                        <dd style="margin:0"><span class="badge bg-{{ $ticket->priority_badge }}">{{ ucfirst($ticket->priority) }}</span></dd>
                    </div>
                    <div>
                        <dt style="color:#94a3b8;font-size:.7rem;text-transform:uppercase;letter-spacing:.5px;font-weight:700">Status</dt>
                        <dd style="margin:0"><span class="badge bg-{{ $ticket->status_badge }}">{{ $ticket->status_label }}</span></dd>
                    </div>
                    @if($ticket->assignedTo)
                    <div>
                        <dt style="color:#94a3b8;font-size:.7rem;text-transform:uppercase;letter-spacing:.5px;font-weight:700">Assigned To</dt>
                        <dd style="margin:0;color:#334155;font-weight:600">{{ $ticket->assignedTo->name }}</dd>
                    </div>
                    @endif
                    <div>
                        <dt style="color:#94a3b8;font-size:.7rem;text-transform:uppercase;letter-spacing:.5px;font-weight:700">Opened</dt>
                        <dd style="margin:0;color:#64748b">{{ $ticket->created_at->format('d-m-Y') }}</dd>
                    </div>
                    @if($ticket->resolved_at)
                    <div>
                        <dt style="color:#94a3b8;font-size:.7rem;text-transform:uppercase;letter-spacing:.5px;font-weight:700">Resolved</dt>
                        <dd style="margin:0;color:#64748b">{{ $ticket->resolved_at->format('d-m-Y') }}</dd>
                    </div>
                    @endif
                </dl>
            </div>
        </div>

        {{-- Close ticket --}}
        @if($ticket->is_open)
        <div class="form-card" style="border-color:#e2e8f0">
            <div class="form-card-body">
                <p style="font-size:.82rem;color:#64748b;margin-bottom:.75rem">
                    Has your issue been resolved? You can close this ticket.
                </p>
                <form action="{{ route('staff.tickets.close', $ticket) }}" method="POST"
                      onsubmit="return confirm('Close this ticket?')">
                    @csrf @method('PATCH')
                    <button type="submit" class="btn btn-outline-secondary btn-sm w-100">
                        <i class="bi bi-check-circle me-1"></i>Mark as Resolved & Close
                    </button>
                </form>
            </div>
        </div>
        @endif

    </div>
</div>
@endsection
